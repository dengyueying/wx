$(function() {
	...
	$(".news-map .list").on("click", "li", function() {
		storage.type = $(this).attr("id");
		storage.html = $(this).children().html();
		mui.openWindow({
			url: 'classify.html',
			id: 'classify',
		});
	...
	//已发布的新闻	
	function published() {
		mui.ajax("http://192.168.1.120:8082/BS56/news/list?status=0&page=" + p0, {
			type: "post",
			dataType: 'json',
	...
	//未读的新闻	
	function unread() {
		mui.ajax("http://192.168.1.120:8082/BS56/news/userSeeStatus?status=1&page=" + p1, {
			type: "post",
			dataType: 'json',
			success: function(data) {
				console.log(JSON.stringify(data));
	...
	//已读的新闻
	function readed() {
		mui.ajax("http://192.168.1.120:8082/BS56/news/userSeeStatus?status=0&userId=7016&page=" + p2, {
			type: "post",
			dataType: 'json',
			success: function(data) {
				console.log(JSON.stringify(data));
	...
	//分类
	function classify() {
		mui.ajax("http://192.168.1.120:8082/BS56/news/newsShorList?page=" + p3, {
			type: "post",
			dataType: 'json',
			success: function(data) {
				console.log(JSON.stringify(data));
				if(data.total == 0) {
					str = '<p class="tswz">您还没有分类</p>';
